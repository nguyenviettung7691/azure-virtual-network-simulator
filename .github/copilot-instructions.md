...
- **Auto-layout uses `parentNode` for visual containment**. `lib/dagre.ts` runs Dagre with a compound graph and assigns `parentNode` (and relative positions) for the real containment hierarchy only: Subnets are children of their VNet; subnet-hosted resources (NICs, VMs, VMSS, AKS, NVA, APP_SERVICE, FUNCTIONS, VPN_GATEWAY, APP_GATEWAY, BASTION, PRIVATE_ENDPOINT, and SERVICE_ENDPOINT) are children of their Subnet; Firewall keeps its direct `vnetId` containment path. The system-managed **Public Internet** node is always kept as a root-level node and is repositioned into a dedicated top row above the rest of the graph after the Dagre pass. NSGs, ASGs, and UDRs are **not** parented to the VNet; they stay as top-level policy nodes and are placed outside the VNet based on the barycenter of the Subnets or NIC-resolved Subnets they protect. When several policy anchors are dense, the outside-policy pass splits them across staggered lanes above the VNet instead of forcing a single row; this reduces edge overlap for NSG-, ASG-, and UDR-like roles as a best effort. Two reverse-lookup maps (`subnetToVnet` and `nicToSubnet`) are built before graph setup for that resolution. After `dagre.layout()`, semantic reflow passes first pack each Subnet, then repack the VNet using the computed Subnet sizes, shifting moved subnet subtrees with their parent so workloads remain enclosed. Reserved top clearance keeps child nodes away from VNet and Subnet header text, and NIC-linked resources render beside their NIC instead of underneath it. Shared sizing constants live in `lib/layout.ts`; VNetNode.vue and SubnetNode.vue must fill the Vue Flow node wrapper (`width: 100%; height: 100%`) rather than sizing themselves from custom `width` / `height` props, otherwise the main canvas can fall out of sync with the minimap for nested containers. The `autoLayout()` store action **removes containment edges** post-layout — it builds a `parentMap` from the new nodes and filters any edge where `parentMap.get(source) === target` or vice versa, since the relationship is already expressed visually. Remaining attachment edges are treated as read-only rendered relationships rather than user-editable connectors. Topological sort ensures parents precede children (Vue Flow requirement). `extent: undefined` — children are not locked inside parents. **Cross-cluster edges are filtered** in the `edges.forEach` block with `if (sourceParent !== targetParent) return` — dagre's compound-graph ranker cannot traverse cluster boundaries and would otherwise throw `"Cannot set properties of undefined (setting 'rank')"`. The `autoLayout()` store action is also wrapped in try/catch.
...
- **Connection-test Animation mode is a temporary rendered overlay, not persisted diagram state.** `stores/diagram.ts` owns `viewMode` (`'infrastructure' | 'animation'`) and an ephemeral `animationSession` used only for canvas rendering. `components/layout/LeftPanel.vue` shows a **Run animation** action for connection tests only; clicking it reruns that test first via `testsStore.runTest(...)`, then calls `diagramStore.playConnectionAnimation(...)` only when the fresh result contains a real traversal path. If the rerun returns a warning with no path, the canvas stays in Infrastructure mode. `DiagramCanvas.vue` swaps its rendered edges to `animationSession.overlayEdges`, decorates node wrapper classes from `animationSession.nodeStates`, forces the canvas into read-only interaction while animating, and shows a fixed, top-centered **Exit animation** button that calls `diagramStore.stopAnimation()`.
- **Animation edges are path-only and directional.** Infrastructure mode continues to render the persisted `diagramStore.edges` through `components/diagram/edges/NetworkEdge.vue`. Animation mode renders only the selected connection test traversal path through `components/diagram/edges/AnimationEdge.vue`, including synthesized containment hops that are normally represented by `parentNode` / metadata instead of persisted edges. The overlay edges are directional from source to target, show a moving paper-plane traveler on the active hop, and leave pass / fail / warning styling on traversed edges and nodes after the traveler disappears.
...
- **Network Tests panel button layout**: The "Add Test" button is full-width (block style, `width: 100%`). Beneath it, "Run All" (`severity="success"`) and "Delete All" (`severity="danger"`) sit side-by-side in a `.action-row` flex container, each with `flex: 1` (`half-btn` class). The three summary tags (Pass / Fail / Pending) in `.test-summary` are center-aligned via `justify-content: center`. Each `.test-item` card has `border-left: 3px solid var(--primary)`. The card has two rows: `.test-header-row` (type icon + test name, separated by a bottom border) and `.test-footer-row` (uppercase result `<Tag>` on the left, Run/Edit/Delete buttons on the right). Below the footer, `.result-message-wrap` shows the result message text, then `.test-flowchart` (when a non-pending/running result exists) renders an icon+label inline flowchart with `overflow-x: auto` horizontal scroll. `getFlowPath(test: NetworkTest): FlowNode[]` drives the flowchart; `FlowNode` has `{ label, icon, blocked?, success? }`. Connection tests: `result.path` entries (node IDs or the string `'Internet'`) are resolved via `resolveFlowItem` (looks up name+icon from `diagramStore.nodes`; falls back to `'Internet'`/`mdi:web` or generic `mdi:server`); for a failing test `blocked: true` is set on the last node, which renders a red `mdi:close-circle` badge (`.flow-blocked-badge`) and colours the main icon red (`.fni-blocked`), while for a passing test `success: true` is set on the last node, which renders a green `mdi:check-circle` badge (`.flow-success-badge`) and colours the main icon green (`.fni-success`). Load-balance: source→LB→backend VMs. Security: NSG count→Subnet count. DNS: DNS Client→zone name→resolved IP (parsed from result.message). `buildInternetTraversalPath(targetNode, blockingNsgId, nodes)` builds the physical traversal path stored in `result.path` for internet connection tests: `['Internet', vnetId, subnetId, subnetNsgId?, nicId, nicNsgId?, targetId?]` — stops at `blockingNsgId` (inclusive) for fails, appends target for passes. Per-test Run button gets a green hover via `.run-test-btn:hover { color: var(--success) }`.
- **Network Summary panel grouped views**: `RightPanel.vue` uses `summaryNodes` to exclude the system-managed **Public Internet** node from the user-facing Components totals and grouped component cards, then derives `groupedComponents` from that filtered list. `groupedConnections` still groups real and synthesized containment edges by `target`, returning `{ targetId, targetName, edges }[]`; it synthesises virtual containment edges from `node.parentNode` because `autoLayout()` removes explicit containment edges from `diagramStore.edges` and expresses them via `parentNode` instead. The Components AccordionPanel renders a `.component-group` card per type, each with a `.group-header` (type icon + label + `<Tag severity="info">` count badge) and a list of component names. The Connectivity AccordionPanel renders a `.conn-table` — a two-column HTML table with "Source" and "Target" column headers; the target cell spans multiple rows (`rowspan`) when a target has more than one source, using `groupedConnections` with a nested `v-for` in a `<template v-for="(group, gi)">` block; the outer index `gi` is used to apply `.conn-group-alt` (sets `background: var(--surface-hover)` on ALL cells via `.conn-table tr.conn-group-alt td`) for alternating group shading, and `.conn-group-first` (sets `border-top: 2px solid var(--border)`) as a visual separator between groups. `.conn-target-cell` has `border-left: 2px solid var(--primary)` and no fixed background. Both source and target cells render a `.conn-cell-content` flex div: a small coloured `<Icon>` (`.conn-cell-icon`) before a `<span>` with the component name; the icon colour and Iconify icon string come from `getNodeIcon(id)` / `getNodeColor(id)` helpers; `v-tooltip="getNodeTypeLabel(id)"` shows the full type label on hover. These three helpers call the existing `getIcon` / `getColor` / `getLabel` wrappers after resolving the node type via `getNodeType(id): NetworkComponentType | null`.
