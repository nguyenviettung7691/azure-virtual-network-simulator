...
const nodeDetail = computed(() => {
  const d = props.data
  if (d.type === NetworkComponentType.STORAGE_ACCOUNT) return `${d.replication || 'LRS'}`
  if (d.type === NetworkComponentType.BLOB_STORAGE) return 'Blob Storage'
  if (d.type === NetworkComponentType.MANAGED_DISK) return `${d.diskSizeGb || 128} GB`
  return ''
})
