  ...
  <div class="component-form">
    ...
    <template v-if="isManagedIdentity">
      <div class="field">
        ...
        <small class="help-text">
          <span v-if="model.identityType === 'SystemAssigned'">
            System-assigned: Created with parent resource, deleted with resource, can only be assigned to one resource.
          </span>
          <span v-else>
            User-assigned: Standalone, reusable, assignable to multiple resources, must be deleted manually.
          </span>
        </small>
      </div>
      <div class="field"><label>Client ID</label>
        <InputText v-model="model.clientId" class="w-full" placeholder="00000000-..." />
        <small class="help-text">Azure application (client) ID. Auto-generated by Azure.</small>
      </div>
      <div class="field"><label>Principal ID</label>
        <InputText v-model="model.principalId" class="w-full" placeholder="00000000-..." />
        <small class="help-text">Service principal object ID in Microsoft Entra ID. Auto-generated by Azure.</small>
      </div>
      <div class="field"><label>Tenant ID</label>
        <InputText v-model="model.tenantId" class="w-full" placeholder="00000000-..." />
        <small class="help-text">Microsoft Entra tenant ID (required for cross-tenant RBAC scenarios).</small>
      </div>
      ...
      <div v-if="model.identityType === 'UserAssigned'" class="field">
        ...
        <small class="help-text">Regional: restricts assignment to resources in the same region. None: assignable across regions.</small>
      </div>
      <div v-if="model.identityType === 'SystemAssigned'" class="field">
        ...
        <small class="help-text">Parent resource (e.g., VM, App Service) this system-assigned identity is attached to.</small>
      </div>
    </template>
  </div>
</template>
