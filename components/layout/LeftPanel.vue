                    ...
                    <template v-for="(fnode, ni) in getFlowPath(test)" :key="ni">
                      <div class="flow-node">
                        <div class="flow-node-icon-wrap">
                          <Icon :icon="fnode.icon" :class="['flow-node-icon', { 'fni-blocked': fnode.blocked, 'fni-success': fnode.success }]" />
...
const TEST_TYPE_ORDER: TestType[] = ['connection', 'loadbalance', 'security', 'dns']
...
async function runAnimation(test: NetworkTest) {
  if (test.type !== 'connection' || isRunAnimationDisabled(test)) return
...
function isRunAnimationDisabled(test: NetworkTest) {
  return test.type !== 'connection'
    || diagramStore.viewMode === 'animation'
...
function getFlowPath(test: NetworkTest): FlowNode[] {
  ...
  if (test.type === 'loadbalance') {
    ...
    if (backends.length > 0) {
      ...
      flowNodes.push({ label: names + (backends.length > 2 ? ` +${backends.length - 2}` : ''), icon: 'mdi:server-network' })
    }
    return flowNodes
  }

  if (test.type === 'security') {
    const nsgCount = diagramStore.nodes.filter(n => n.data?.type === NetworkComponentType.NSG).length
    const subnetCount = diagramStore.nodes.filter(n => n.data?.type === NetworkComponentType.SUBNET).length
    const flowNodes: FlowNode[] = []
    if (nsgCount > 0) flowNodes.push({ label: `${nsgCount} NSG${nsgCount > 1 ? 's' : ''}`, icon: 'mdi:shield-check' })
    if (subnetCount > 0) flowNodes.push({ label: `${subnetCount} Subnet${subnetCount > 1 ? 's' : ''}`, icon: 'mdi:lan' })
    return flowNodes
  }
  ...
  if (test.type === 'dns') {
    ...
    flowNodes.push(ipMatch ? { label: ipMatch[0], icon: 'mdi:ip-network' } : { label: 'Resolved', icon: 'mdi:check-circle' })
    return flowNodes
  }
